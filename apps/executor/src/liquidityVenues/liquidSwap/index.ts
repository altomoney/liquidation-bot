import { LIQUID_SWAP_SUPPORTED_NETWORKS } from "@/config";
...
export class LiquidSwapVenue implements LiquidityVenue {
  ...
  async convert(encoder: ExecutorEncoder, toConvert: ToConvert) {
    ...
    try {
      ...
      const url = this.apiUrl(
        src,
        dst,
        Math.floor(Number(srcAmount) / 10 ** srcDecimals)
      );
  ...
  private async getAssetsDecimals(
    ...
    if (this.assetsDecimals[client.chain.id]![asset] === undefined) {
      this.assetsDecimals[client.chain.id]![asset] = await readContract(
        client,
        {
          address: asset,
          abi: erc20Abi,
