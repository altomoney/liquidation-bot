...
export class UniswapV4Venue implements LiquidityVenue {
    ...
    Hex,
    {
      events: Awaited<
        GetContractEventsReturnType<
          typeof uniswapV4PoolManagerAbi,
          "Initialize",
          true
        >
      >;
  ...
  async convert(encoder: ExecutorEncoder, toConvert: ToConvert) {
    ...
    const { currency0, currency1, pools } = await this.fetchPools(
      encoder,
      PoolManager,
      src,
      dst,
    ...
    encoder.pushCall(
      ...
      encodeFunctionData({
        ...
        args: [
          rawSrc,
          deployments.UniversalRouter.address,
          srcAmount,
          Number(deadline),
        ],
      }),
  ...
  private async fetchPools(
    ...
    const [currency0, currency1] =
      BigInt(src) < BigInt(dst) ? [src, dst] : [dst, src];

    const cache = this.poolCreationEventsCache[`${currency0}${currency1}`];
    const cacheIsValid =
      cache?.lastUpdate && Date.now() - cache.lastUpdate < this.STALE_TIME;
