...
import {
  oneInchTbtcUsdcFixture,
  TBTC_ONE_INCH_FORK_BLOCK_NUMBER,
} from "@/test/fixtures/oneInchTbtcUsdc";
import {
...
const CHAIN_ID = 1;
const MARKET_ADDRESS = "0xE17F10f8535336143d4F2FE304f3Dcf7B23671e1";
...
beforeEach(() => {
  ...
  nock("https://api.1inch.dev")
    ...
    .query(true)
    .reply(200, oneInchTbtcUsdcFixture.swap);
});

describe.sequential("tBTC liquidation fork test (1inch)", () => {
  executionTest(
    ...
    async ({ client, encoder }) => {
      ...
      const collateralAmount = 2n * 10n ** 17n; // 0.2 tBTC
      const bufferedSwapAmount = (collateralAmount * 995n) / 1000n;
      console.log(
        "[test-tbtc-1inch] Fixture swap",
        `${bufferedSwapAmount.toString()} -> ${oneInchTbtcUsdcFixture.dstAmount} USDC units`,
      );
