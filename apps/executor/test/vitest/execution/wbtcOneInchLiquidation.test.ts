...
import {
  oneInchWbtcUsdcFixture,
  WBTC_ONE_INCH_FORK_BLOCK_NUMBER,
} from "@/test/fixtures/oneInchWbtcUsdc";
import {
...
beforeEach(() => {
  ...
  nock("https://api.1inch.dev")
    ...
    .query(true)
    .reply(200, oneInchWbtcUsdcFixture.swap);
});
  ...
  executionTest(
    ...
    async ({ client, encoder }) => {
      ...
      console.log(
        ...
        `${bufferedSwapAmount.toString()} WBTC sats -> ${oneInchWbtcUsdcFixture.dstAmount} USDC units`,
      );
