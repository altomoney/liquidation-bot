...
import {
  oneInchSyrupusdcUsdcFixture,
  SYRUPUSDC_ONE_INCH_FORK_BLOCK_NUMBER,
} from "@/test/fixtures/oneInchSyrupusdcUsdc";
import {
...
beforeEach(() => {
  ...
  nock("https://api.1inch.dev")
    ...
    .query(true)
    .reply(200, oneInchSyrupusdcUsdcFixture.swap);
});

describe.sequential("syrupUSDC liquidation fork test (1inch)", () => {
  executionTest(
    ...
    async ({ client, encoder }) => {
      ...
      console.log(
        "[test-syrupusdc-1inch] Fixture swap",
        `${bufferedSwapAmount.toString()} syrupUSDC sats -> ${oneInchSyrupusdcUsdcFixture.dstAmount} USDC units`,
      );
